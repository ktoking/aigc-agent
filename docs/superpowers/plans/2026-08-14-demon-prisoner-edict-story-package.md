...
- 每段使用 4 个视频导演镜头，由 3 张关键分镜图提供参考；允许同一参考图承担连续动作或二次构图，不新增第 4 张图片。
- `storyboard.md`、`prompt.md`、`director-promt.txt` 采用 `forest-villa-apocalypse` 成品导演剧本格式，并保持生产正文一致。
- 本计划不提交任何付费视频 API 任务。
...
- [x] **Step 1: 写入 Segment 01 和 02**

每段包含 4 个视频导演镜头，由 3 张已有关键分镜图提供参考；按分镜而非机械逐秒组织正文，同时在导演提示词中保留镜头次序、动作因果和总时长约束。

- [x] **Step 2: 写入 Segment 03 和 04**

Segment 03 明确“放箭 → 沈烬抬眼说停 → 箭、雨、刀同时凝固”；Segment 04 明确“牵手穿行 → 眼角渗血 → 时间恢复 → 妖王影子显现”。

- [x] **Step 3: 补齐首尾帧职责和 API 记录模板**
