...
- 若提交多张图，必须按 API 输入顺序写成“参考图1/参考图2/参考图3”，逐张说明用途；不要笼统写“参考所有图片”。
- 写清角色锁、道具锁、禁止变化项。
...
每段默认自动参考图只包含 `frames/first-frame.png` 和 `frames/last-frame.png`。故事板表格图不默认上传，避免模型把分镜表、文字、箭头或表格布局当成画面内容。

正式提交时，推荐每段显式选择 3-5 张图：

- 参考图1：本段首帧，锁定 0 秒桥接画面。
- 参考图2：本段尾帧，锁定 15 秒收束画面。
- 参考图3：本段最容易漂移的对象，例如角色脸、角色三视图、动作编排、场景路线或关键道具。
- 参考图4-5：只在确有必要时追加，例如复杂打斗、精确空间路线、六块旧表数量、怀表比例。

如果参考图是路线图、动作图、道具比例图或带标注的生产图，`director-promt.txt` 必须写清“只用于理解空间/动作/比例，不要把箭头、文字、网格、标注画进最终视频”。
