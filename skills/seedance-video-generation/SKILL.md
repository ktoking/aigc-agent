...
- 默认可使用本段 `frames/first-frame.png` 和 `frames/last-frame.png`；但 story 的生产规则或用户明确要求无首尾帧模式时，禁止传入二者和 `--auto-images`，改为显式传入无人物场景、特效或道具资产。角色一致性优先用数字人资产与完整服装约束。
...
- 无首尾帧模式不要只依赖一张综合参考图：普通段使用 2 张、复杂动作段使用 3 张非人物参考图，分别锁定场景结构、关键特效、关键道具或状态；默认上限为 3 张，避免参考职责互相冲突。
- 外景出现固定建筑时，首张非人物参考图必须锁定 canonical 建筑；特效图和道具图不能替代建筑结构图。
- 只传本段实际需要清晰锁脸的数字人。多名同性别人物同时出现时，按首次清晰露脸顺序传入，不能把未露脸角色的资产一起塞进请求。
...
3. 确认图片策略：
   - 常规模式检查 `frames/first-frame.png`、`frames/last-frame.png`；无首尾帧模式确认二者不被传入且不启用 `--auto-images`。
   - 确认本段至少一张明确职责的无人物场景、特效或道具资产存在。
   - 角色三视图/定妆图，如 `frames/**/character-turnaround.png`，仅在该 story 允许时传入。
   - 故事板表格图仅用于人工核对，不默认上传。
...
1. 常规模式：参考图1 为 `frames/first-frame.png`，参考图2 为 `frames/last-frame.png`。
2. 无首尾帧模式：参考图1 直接使用本段最容易漂移的无人物资产，如场景路线、特效逻辑、动作编排或关键道具；不要传首尾帧。
3. 其余参考图只在复杂打斗、精确空间路线、道具数量或身份反转证据确有必要时追加。
...
- **建筑锚图必传**：当故事有固定别墅、基地、车辆或室内结构时，episode 旧首帧不能作为唯一空间依据；除首尾帧外，必须上传一张当前 canonical 场景锚图。提交前肉眼对比首帧与锚图，若建筑层数、屋顶、围墙、电网、炮台或室内布局不一致，停止提交并先修资产。
...
- **原生音频边界**：Seedance 原生音频优先当作环境声、动作声、群体反应声使用。15 秒动作段最多三句、约六十个有效口语汉字，每个镜头只允许一个可见说话人；特效、追逐、打斗或转场镜头优先无台词。不要在一个 15 秒任务里依赖多角色精准对白和精确说话人归属；如果台词归属很重要，优先后期配音/剪辑，或只保留一个可见说话人、一句短台词，并明确其他角色全程不说话。
